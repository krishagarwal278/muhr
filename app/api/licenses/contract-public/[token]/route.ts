...
export async function GET(
  ...
  if (row.status !== "accepted") {
    return NextResponse.json({ error: "This request is not active" }, { status: 400 });
  }
...
export async function POST(
  ...
  if (legal.length < 2 || legal.length > 200) {
    return NextResponse.json({ error: "legal_name must be 2–200 characters" }, { status: 400 });
  }
  ...
  if (!row.creator_signed_contract_at) {
    return NextResponse.json(
      { error: "The creator must sign before you can sign" },
      { status: 400 }
    );
  }
  ...
  if (updErr) {
    console.error("brand sign:", updErr);
