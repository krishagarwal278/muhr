...
export function VaultGridAssetCard({
  ...
  const allBadges = computeSecurityBadges(toSecurityInput(asset), creator);
  const badges = allBadges.filter((b) => b.key !== "last_accessed");
...
export function VaultRowAssetCard({
  ...
  const allBadges = computeSecurityBadges(toSecurityInput(asset), creator);
  const badges = asset.asset_type === "document"
    ? allBadges.filter((b) => b.key === "encrypted")
    : allBadges;
    ...
    >
      <div className="flex items-center gap-3 p-4">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium">{asset.original_file_name ?? asset.file_name}</p>
